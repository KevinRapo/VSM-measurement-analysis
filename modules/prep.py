...
class MeasurementObj:
    ...
    def return_path(self):
        return self.file_path, self.user_path
    ...
    def checkMeasurementType2(self, measurement_table, discrete_detection_ration=0.02, min_count=5):
        ...
        rounded_dataset_H = measurement_table["Magnetic Field (Oe)"]
        ...
        if ratio_T < discrete_detection_ration:  # discrete
            ...
            if ratio_H < discrete_detection_ration:  # discrete
                ...
                temperatures_of_interest = pd.concat(
                    [temperatures_of_interest, pd.Series(tempCount.index.values)], ignore_index=True)
            ...
            if ratio_H < discrete_detection_ration:  # discrete
                ...
                fieldCount = fieldCount[fieldCount > min_count]
                magnetic_fields_of_interest = pd.concat(
                    [magnetic_fields_of_interest, pd.Series(fieldCount.index.values)], ignore_index=True)
            else:  # continous
    
                print("T continous, H continous = both \n")
    
                # see osa siin annab segafailide puhul mõistlikud numbrid. Aga mõne erilisema faili korral ei saa ta aru et sega fail on, aga need read annavad ka siis mõistlikud numbrid. Äkki saame kuidagi hoopis neid prinditavaid liste kasutada faili ära tundmiseks.
                meanTempCount = tempCount.mean()
                muutuja = rounded_dataset_T.value_counts() > meanTempCount*10
                temperatures_of_interest = pd.Series(
                    rounded_dataset_T.value_counts()[muutuja].index.values)
                print("meanTempCount", meanTempCount)
                meanFieldCount = fieldCount.mean()
                muutuja2 = rounded_dataset_H.value_counts() > meanFieldCount*10
                magnetic_fields_of_interest = pd.Series(
                    rounded_dataset_H.value_counts()[muutuja2].index.values)
...
class MvsH:
    ...
    def CorrectionTableToDict(self, correctionField_values, user_path):
        ...
        for nr in correctionField_values:
            correction_table_path = self.searchCorrectionTable(
                correction_folder_path, nr)

            if correction_table_path is None:
                print(f"{nr} Oe jaoks ei leia parandustabelit ")
                continue
            correction_table_measurements = pd.read_csv(
                correction_table_path, index_col=0, encoding="ANSI")
...
def separationIndexForSingleSeries(original_dataframe, indices, column_name, const, x=0.1):
    """
...
def setPointsColor(original_dataframe, pairs, colors):
    ...
    for pair in pairs:
        ...
        # if the colors are used up it goes back to the beginning
